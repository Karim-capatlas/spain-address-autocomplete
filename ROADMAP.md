...
**Success:** Repo is live at https://github.com/Karim-capatlas/spain-address-autocomplete — **190 tests passing** (17 files), typecheck 9/9 green, lint 0 errors.
...
- [x] **All 190 tests pass**; street search live-verified against local Typesense with the full 749,261-record dataset ("Gran Vía" → **131** national hits; CP-28013 + "mayor" → exactly `Calle Mayor, Madrid`)
...
**Success:** Standalone MCP server normalizes address strings against the real 749,261-record INE dataset using the default local Typesense store, over stdio or HTTP. ✅ Achieved (`docker compose up -d typesense` → `pnpm --filter @spain-address/mcp start`, or `start:http` for `/mcp`).
...
- [x] MCP **Streamable HTTP** transport as a systemd service (`spain-mcp`, :8789, `/mcp`) + Tunnel route — see `docs/vps-deploy.md`
